...
void setup()
{
    ...
    //button1.begin();
    //button1.changeEffect(onPressed);
...
void changeEffect()
{
    ...
    case 6:
        ...
        drawLargeText("Spiro");
        break;
        // Always make Random the last effect, because the random code only generates a random effect up to (PATTERN_COUNT - 1)          
    case 7:
        // if the user selects random and is idle for WAIT_ON_USER_SECONDS, start the random effect mode 
...
void readPot()
{
    ...
    if (abs(newBrightness - brightness) > 3)
    {
        // deadzone
        brightness = newBrightness;
        if (debugPot) Serial.print("****** set brightness: ");
        if (debugPot) Serial.println(brightness);
