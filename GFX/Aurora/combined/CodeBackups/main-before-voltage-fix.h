...
#define WAIT_ON_RETURN_TO_DEFAULT_DISPLAY_SECONDS 1
...
void loopTicker();
void readPot();
void setupAverageBrightness();
int getAveragePotValue();
void setupDisplay();
...
void readPot() {
    ...
    drawSmallText((brightnessString + brightnessValue).c_str());
        ...
        if (newBrightness > 0 || newBrightness <= 255) {
            ...
            //hardcode during testing, wifi threw it off!
            matrix->setBrightness(newBrightness);
        }
...
void readPot2()
{
    // circuit is a bit noisy, so require more input to "activelyAdjustBrightness" which shifts into a lower deadzone.  The timer will turn it off
    if (activelyAdjustingBrightness) {
      deadzone = 1;
      // keep the avg going, but use the immediate value
      getAveragePotValue();  
      potValue = analogRead(potPin);
    } else {
      deadzone = 10;
      potValue = getAveragePotValue();
    }
    ...
    if (abs(newBrightness - brightness) > deadzone)
    {
        ...
        // this prevents the brightness display showing up on boot
        if (brightness > 0) {
...
bool drawBlinkyDot(void *)
{             
    // prevent the blinking the dot from flickering the ticker ...
    if (tickerModeOn) return true;
